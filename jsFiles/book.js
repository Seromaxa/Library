`use strict`
let newBook = document.getElementsByName(`chuser`)[0]; // Кнопка для слушателя
...
let bookCreater = document.forms[`new book`]; // ФОРМА
let sortButt = document.getElementsByClassName(`button`)[0];
let library =  catcher(`book`,`library`)
...
listCreater()
search (0)
opener (newBook,plait)
validAmount ()

bookCreater.addEventListener(`submit`, function (event) {
    checkClass(bookCreater,`red`)
       let checker = showErrors ();
        if (checker == false){
...
function creatBook (){
    ...
    name1 = name1.split(``)[0].toUpperCase() + name1.substr(1).toLowerCase();
    let book = {
        item: 'book',
        genre: document.getElementsByClassName(`genre`)[0].value,
        name: name1,
        author: upper( document.getElementById(`author`).value),
        yearOfPablish: document.getElementsByClassName(`date`)[0].value,
        pablishHouse: upper(document.getElementById(`PH`).value),
...
function idCreater (){
    ...
    autor = autor.split(``)[0].toUpperCase();
   ...
   let num = makeANum (`book`)+ 1;
    
 return  bookCreater.ind.value = autor + name + genre + `/` + num;
...
return localStorage.setItem(`book`+ number + name,JSON.stringify(book));
}

function showErrors (){ 
    let checker =  validSelect ()
   
      for(let counter = 0; counter < bookCreater.length;counter++){
          if(bookCreater[counter].value == ""){
  bookCreater[counter].previousElementSibling.classList = `red`;
  }
      }
  for(let counter = 0; counter<bookCreater.length;counter++){
      if(bookCreater[counter].value == "" || checker == false){
       return false
      }
  }
  }
  ...
  function validSelect (){
    let checker;
    let optGenre = document.getElementById(`genre`);
   let optYear = document.getElementById(`year`);
    if(optGenre.value == `Выберите жанр`){
        optGenre.previousElementSibling.classList = `red`;
        checker = false
    }
    if(optYear.value == `Выберите год`){
        optYear.previousElementSibling.classList = `red`;
        checker = false
    }
   return checker
}
...
let catcher = JSON.parse(localStorage.getItem(`library`))
...
return localStorage.setItem(`library`,JSON.stringify(catcher))
}
...
function changer (){
    let catcherCH = JSON.parse(localStorage.getItem(`library`))
    let changeForm = document.forms[`changeForm`];
        for(let key in catcherCH){
            if(catcherCH[key].id === this.value){
             changeForm.genre_change.value = catcherCH[key].genre;
             changeForm.author_change.value = catcherCH[key].author;
             changeForm.PH_change.value = catcherCH[key].pablishHouse;
             changeForm.name_change.value = catcherCH[key].name;
             changeForm.amount_change.value = catcherCH[key].amount;
             changeForm.year_change.value = catcherCH[key].yearOfPablish
             changeForm.pages_change.value = catcherCH[key].pages   

changeForm.addEventListener(`submit`,function(){  
    catcherCH[key].genre = changeForm.genre_change.value ;
    catcherCH[key].author = upper(changeForm.author_change.value);
    catcherCH[key].pablishHouse = upper(changeForm.PH_change.value);
    catcherCH[key].name = changeForm.name_change.value;
    catcherCH[key].amount = changeForm.amount_change.value;
    catcherCH[key].yearOfPablish = changeForm.year_change.value;
    catcherCH[key].pages = changeForm.pages_change.value ;
    return  localStorage.setItem(`library`,JSON.stringify(catcherCH));
...
function youreChoise(){  
  ...
  forOfchuse.innerHTML = `<p class = ask> Вы действительно хотите изменить книгу</p>`
...
function listCreater(){
    let listHolder = document.getElementsByClassName(`tabHead`)[0];
    let booker = JSON.parse(localStorage.getItem(`library`));
    if(booker == null){
        booker = {}
    ...
    for(let counter = 0;counter < number;counter++){
        ...
        let listItem6 = document.createElement(`td`);
...
delButton.addEventListener(`click`, deleter);
        ...
        listItem1.innerText = booker[ind[counter]].author;
        listItem2.innerText = booker[ind[counter]].name;
        listItem3.innerText = booker[ind[counter]].genre;
        listItem4.innerText = booker[ind[counter]].amount;
        ...
        tabR.append(listItem5);
        tabR.append(listItem6);


        listItem5.append(changeButton);
listItem6.append(delButton);
changeButton.value = booker[ind[counter]].id;
delButton.value = booker[ind[counter]].id
changeButton.innerHTML = `<svg version="1.1" id="Capa_2" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" x="0px" y="0px"
...
delButton.innerHTML = `<svg version="1.1" id="Capa_1" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" x="0px" y="0px"
...
function validAmount (){
    let listHolder = document.getElementsByClassName(`tabHead`)[0];
    for(let counter = 0;counter < listHolder.rows.length;counter++){
        if(listHolder.rows[counter].cells[4].innerText === `0`){
            listHolder.rows[counter].classList.add(`red`);
        }else{
            listHolder.rows[counter].classList.remove(`red`);
